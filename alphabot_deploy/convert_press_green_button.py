#!/usr/bin/env python3
"""
Convert custom H5 format to LeRobot V2 format for GR00T fine-tuning (Right Arm Version, No Gripper)

Input format (from H5 files):
  - observations/right_arm_joint_status (T, 7) - 7 joint angles (degrees)
...
def load_h5_episode(h5_path: Path, camera_names: list[str]) -> dict[str, Any] | None:
    ...
    try:
        with h5py.File(h5_path, "r") as f:
            ...
            if "observations/right_arm_joint_status" not in f:
                print(f"  Error: Missing right_arm_joint_status")
                return None

            # Read joint data
            joint_status = f["observations/right_arm_joint_status"][()]  # (T, 7)

            if joint_status.shape[0] == 0:
                print(f"  Skip: Empty right_arm_joint_status")
                return None
            ...
            # Truncate joint data to final episode length
...
def write_video(frames: np.ndarray, output_path: Path, fps: int = 30):
    """
    Write frames to MP4 video file.

    Args:
        frames: (T, H, W, 3) RGB uint8 array
...
def save_episode_video(images: np.ndarray, output_path: Path, fps: int = 30):
    """
    Write frames to MP4 video file using OpenCV.

    Args:
        images: (T, H, W, 3) RGB frames
...
def compute_statistics(all_qpos: list[np.ndarray], all_actions: list[np.ndarray]) -> dict:
    ...
    # Stack all episodes
    qpos_stack = np.concatenate(all_qpos, axis=0)  # (N, 7)
    action_stack = np.concatenate(all_actions, axis=0)  # (N, 7)
...
def create_modality_config(camera_names: list[str]) -> dict:
    """Create modality configuration for LeRobot V2"""
    video_keys = [f"video.{CAMERA_NAME_MAP[cam]}" for cam in camera_names]
    ...
    return {
        ...
        "delta_indices": {
            "state": [0],
            "action": list(range(16)),  # 16-step action horizon
            "video": [0],
        },
    }
...
def convert_dataset(
    ...
    task_description: str = "Press green button with right arm",
    fps: int = 30,
    ...
    for episode_idx, h5_path in enumerate(tqdm(h5_files, desc="Converting episodes")):
        ...
        joint_status = episode_data["joint_status"]
        images = episode_data["images"]
        length = episode_data["episode_len"]
        ...
        for cam_name, img_data in images.items():
            video_key = f"observation.images.{CAMERA_NAME_MAP[cam_name]}"
            video_path = video_dir / f"chunk-{chunk_idx:03d}" / video_key / f"episode_{episode_idx:06d}.mp4"
            write_video(img_data, video_path, fps=fps)
    ...
    # Use mapped camera names (front, wrist_right)
    ...
    # Generate info.json
    # Use mapped camera names (front, wrist_right) instead of original names
    mapped_camera_names = [CAMERA_NAME_MAP.get(cam, cam) for cam in camera_names]
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert H5 dataset to LeRobot V2 format (Right Arm Version)"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument(
        "--task-description",
        type=str,
        default="Press green button with right arm",
